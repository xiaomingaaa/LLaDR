...
relation_mapping_file = 'D:/........./relations.tsv'          # 关系 ID 映射文件  
...
def vectorized_rotate_score(head_embs, relation_embs, tail_embs):
    # 定义常量
    pi = 3.14159265358979323846 
    gamma = 12.0 # 评分任务时使用固定的
    embedding_dim = head_embs.size(1) // 2 # 200维 # 计算
    embedding_range = (gamma + 2.0) / embedding_dim

    # 将嵌入向量分割为实部和虚部
    head_re, head_im = head_embs.chunk(2, dim=1)
    tail_re, tail_im = tail_embs.chunk(2, dim=1)

    # 计算关系的相位
    phase_relation = relation_embs / (embedding_range / pi)
...
def evaluate_triples(triples, entity_mapping, relation_mapping, entity_embeddings, relation_embeddings, tails_pool, device):
    ...
    for idx, triple in triples.iterrows():
        ...
        with torch.no_grad():
            ...
            scores = vectorized_rotate_score(expanded_head, expanded_rel, tail_embs)

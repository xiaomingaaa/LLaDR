...
def vectorized_dist_mult_score(head_embs, relation_embs, tail_embs):  
    """Vectorized scoring function using DistMult with potential GPU acceleration"""  
    # Calculate the scores using DistMult  
...
def evaluate_triples(triples, entity_mapping, relation_mapping, entity_embeddings, relation_embeddings, tails_pool, device):
    ...
    for idx, triple in triples.iterrows():
        ...
        with torch.no_grad():
            ...
            scores = vectorized_dist_mult_score(expanded_head, expanded_rel, tail_embs)

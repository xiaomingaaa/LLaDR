...
def construct_args():
    parser = argparse.ArgumentParser(description='LLaDR')
    # Data paths
    parser.add_argument('--data_path', type=str, default='D:/........./autodl-fs', help='Path to the dataset')
    parser.add_argument('--dataset', type=str, default='DRKG', help='Dataset name')
    parser.add_argument('--process_path', type=str, default='/root/autodl-fs', help='Path to the entity hierarchy')
    parser.add_argument('--log_file_path', type=str, default='/root/log.txt', help='Path to the entity hierarchy')
    # train, valid, test
    ...
    # Model settings
    parser.add_argument('-de', '--double_entity_embedding',default=False, action='store_true')
    parser.add_argument('-dr', '--double_relation_embedding',default=False, action='store_true')
    ...
    parser.add_argument('--model', type=str, default='DistMult', help='Knowledge graph embedding model')
    ...
    parser.add_argument('--cuda', action='store_true',default='true',help='Use GPU for training')
    ...
    args.save_path = f'D:/.........//{args.model}_batch_{args.batch_size}_hidden_{args.hidden_dim}'
...
def log_metrics(mode, step, metrics):  
    # 记录到 WandB  
    wandb.log({f"{mode}_{k}": v for k, v in metrics.items()}, step=step)  
    log_message = f"{mode} step {step}: {metrics}"  
    print(log_message)  
    with open("D:/........./metrics_log.txt", "a") as f:  # 使用 "a" 模式以添加内容  
        f.write(log_message + "\n")  # 添加换行符
def main(args):
    wandb.init(project="LLaDR", config=args, name=f"{args.dataset}-{args.model}-{args.hidden_dim}")
    loss_table = wandb.Table(columns=["text_dist_n", "negative_sample_loss", "positive_sample_loss", "loss"])
    ...
    # Load the entity text embeddings
    ...
    wandb.watch(LLaDR_model)
    ##########################
    
    logging.info('Model Parameter Configuration:')
    for name, param in LLaDR_model.named_parameters():
        logging.info('Parameter %s: %s, require_grad = %s' % (name, str(param.size()), str(param.requires_grad)))
    
    if args.cuda:
        LLaDR_model = LLaDR_model.cuda()
    ...
    if args.do_train:
        ...
        optimizer = torch.optim.Adam(
            filter(lambda p: p.requires_grad, LLaDR_model.parameters()), 
    ...
    if args.init_checkpoint:
        ...
        checkpoint = torch.load(os.path.join(args.init_checkpoint, 'checkpoints'))
        init_step = checkpoint['step']
        LLaDR_model.load_state_dict(checkpoint['model_state_dict'], strict=False)
        if args.do_train:
        ...
        logging.info('Randomly Initializing %s Base Model...' % args.model)
        init_step = 0
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            log = LLaDR_model.train_step(LLaDR_model, optimizer, train_iterator, args)
            ...
            if step % args.save_checkpoint_steps == 0:
                ...
                save_model(LLaDR_model, optimizer, save_variable_list, args)
            ...
            if args.do_valid and step % args.valid_steps == 0:
                ...
                metrics = LLaDR_model.test_step(LLaDR_model, valid_triples, all_true_triples, args)
    ...
    if args.do_test:
        ...
        torch.cuda.empty_cache()
        metrics = LLaDR_model.test_step(LLaDR_model, test_triples, all_true_triples, args)
    ...
    if args.evaluate_train:
        ...
        torch.cuda.empty_cache()
        metrics = LLaDR_model.test_step(LLaDR_model, train_triples, all_true_triples, args)
        log_metrics('Test', step, metrics)
